...
export function ProductMenuForm({ open, onClose, editItem }: ProductMenuFormProps) {
  ...
  const {
    ...
    defaultValues: { name: '', category: '', price: 0, recipeId: '', imageUrl: '', isActive: true },
  });
  ...
  useEffect(() => {
    if (editItem) {
      ...
      reset({ name: '', category: '', price: 0, recipeId: '', imageUrl: '', isActive: true });
    }
  }, [editItem, reset]);
  ...
  useEffect(() => {
    api.get('/recipes').then((r) => setRecipes(r.data)).catch(console.error);
  }, []);
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) { reset(); onClose(); } }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{isEditing ? 'Edit Product Menu' : 'Add Product Menu'}</DialogTitle>
        </DialogHeader>
