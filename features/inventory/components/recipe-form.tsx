...
type FormValues = { name: string; description: string; ingredients: IngredientRow[] };
...
export function RecipeForm({ open, onClose, editItem }: RecipeFormProps) {
  ...
  const { register, handleSubmit, setValue, watch, reset, control, formState: { errors, isSubmitting } } =
    useForm<FormValues>({
      defaultValues: { name: '', description: '', ingredients: [{ rawMaterialId: '', quantity: 0 }] },
    });
  ...
  useEffect(() => {
    if (editItem) {
      ...
      reset({ name: '', description: '', ingredients: [{ rawMaterialId: '', quantity: 0 }] });
    }
  }, [editItem, reset]);
  ...
  useEffect(() => {
    api.get('/raw-materials').then((r) => setRawMaterials(r.data)).catch(console.error);
  }, []);
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 py-2">
          ...
          <div className="grid gap-2">
            ...
            <Input id="rec-desc" placeholder="Brief description..." {...register('description')} />
          </div>
            ...
            {fields.map((field, idx) => (
              <div key={field.id} className="flex gap-2 items-start">
                <Select
                  ...
                  onValueChange={(v) => setValue(`ingredients.${idx}.rawMaterialId`, v)}
                >
