          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>
                  <span className="text-muted-foreground">{item.minimumStock} {item.unit}</span>
                </TableCell>
                <TableCell>
                  ...
                  {item.usedBy && item.usedBy.length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {item.usedBy.map((name) => (
                        <Badge key={name} variant="outline" className="text-[10px] font-normal text-muted-foreground">
                          {name}
                        </Badge>
                      ))}
                    </div>
