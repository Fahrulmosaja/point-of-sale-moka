...
export function InventoryTabs() {
  ...
  // ── Tab state ──────────────────────────────────────────────────────────────
  const [activeTab, setActiveTab] = useState<'raw-materials' | 'product-menus'>('raw-materials');
  ...
  // ── Delete confirm ─────────────────────────────────────────────────────────
  const [deleteTarget, setDeleteTarget] = useState<{ type: 'rm' | 'pm'; id: string; name: string } | null>(null);
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, { type }) => {
      queryClient.invalidateQueries({ queryKey: [type === 'rm' ? 'raw-materials' : 'product-menus'] });
  ...
  return (
    <>
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
        ...
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <TabsList className="h-9">
              <TabsTrigger value="raw-materials" className="gap-2 text-sm">
                Raw Materials
                {rmAlerts > 0 && (
                  <Badge variant="destructive" className="h-4 text-[10px] px-1">{rmAlerts}</Badge>
                )}
              </TabsTrigger>
              <TabsTrigger value="product-menus" className="gap-2 text-sm">
                Product Menus
                {pmAlerts > 0 && (
                  <Badge variant="destructive" className="h-4 text-[10px] px-1">{pmAlerts}</Badge>
                )}
              </TabsTrigger>
            </TabsList>
            ...
            <div className="flex gap-2">
              {activeTab === 'raw-materials' && (
                <Button
                  size="sm"
                  onClick={() => { setRmEdit(null); setRmFormOpen(true); }}
                  className="gap-1.5"
                >
              ...
              {activeTab === 'product-menus' && (
                <>
                  ...
                  <Button
                    size="sm"
                    onClick={() => { setPmEdit(null); setPmFormOpen(true); }}
                    className="gap-1.5"
                  >
          ...
          {statusFilter !== 'all' && (
            <p className="text-sm text-muted-foreground -mt-1">
              ...
              {activeTab === 'raw-materials' ? filteredRm.length : filteredPm.length} item(s)
            </p>
          )}
        </div>
        ...
        <TabsContent value="raw-materials" className="mt-0">
          {rmLoading ? (
            ...
            <RawMaterialsTable
              items={filteredRm}
              onEdit={(item) => { setRmEdit(item); setRmFormOpen(true); }}
              onDelete={(item) => setDeleteTarget({ type: 'rm', id: item.id, name: item.name })}
            />
          )}
        </TabsContent>
        ...
        <TabsContent value="product-menus" className="mt-0">
          {pmLoading ? (
            ...
            <ProductMenuTable
              items={filteredPm}
              onEdit={(item) => { setPmEdit(item); setPmFormOpen(true); }}
              onDelete={(item) => setDeleteTarget({ type: 'pm', id: item.id, name: item.name })}
            />
          )}
        </TabsContent>
      </Tabs>
      ...
      <RawMaterialForm
        open={rmFormOpen}
        onClose={() => { setRmFormOpen(false); setRmEdit(null); }}
        editItem={rmEdit}
      />
      <RecipeForm
        open={recipeFormOpen}
      ...
      <ProductMenuForm
        open={pmFormOpen}
        onClose={() => { setPmFormOpen(false); setPmEdit(null); }}
        editItem={pmEdit}
      />
